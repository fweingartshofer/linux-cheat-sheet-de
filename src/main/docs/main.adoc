...
sed '/<line_pattern>/s/<find>/<replace>/' <file> <3>
sed '/<regex>/d' <file> <4>
sed -n '<n>,/^$/p' <file> <5>
sed 's#<regex>#<replace>#' <file> <6>
...
<3> Ersetzt nur die Matches in den Zeilen, welche das `<line_pattern>` erfüllen
<4> Löscht die Zeilen, welche einen Match haben
<5> Gibt alle Zeilen von `<n>` bis zur nächsten leeren Zeile aus
<6> Ein Slash `/` als Trennzeichen kann auch durch ein anderes ersetzt werden, z.B. eine Raute `#`
